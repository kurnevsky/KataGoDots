...
class Model(torch.nn.Module):
    def __init__(self, config: modelconfigs.ModelConfig, pos_len_x: int, pos_len_y: int, games=None, katago_git_rev: str = None, katago_backend: str = None):
        ...
        if config["initial_conv_1x1"]:
            self.conv_spatial = torch.nn.Conv2d(8, self.c_trunk, kernel_size=1, padding="same", bias=False)
        else:
            self.conv_spatial = torch.nn.Conv2d(8, self.c_trunk, kernel_size=3, padding="same", bias=False)
        ...
        self.bin_input_shape = [8, pos_len_y, pos_len_x]
